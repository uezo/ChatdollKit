...
{
    public class OnnxSileroStreamPartialLoggingTests
    {
        [UnityTest]
        public IEnumerator PartialLoggingDoesNotDelayFinalRecognition()
            => UniTask.ToCoroutine(() => PartialLoggingDoesNotDelayFinalRecognitionAsync());

        private async UniTask PartialLoggingDoesNotDelayFinalRecognitionAsync()
        {
        ...
        [UnityTest]
        public IEnumerator QueuedPartialLogIsDiscardedWhenStopped()
            => UniTask.ToCoroutine(() => QueuedPartialLogIsDiscardedWhenItsSourceIsNoLongerCurrentAsync("stopped"));

        [UnityTest]
        public IEnumerator QueuedPartialLogIsDiscardedWhenReplaced()
            => UniTask.ToCoroutine(() => QueuedPartialLogIsDiscardedWhenItsSourceIsNoLongerCurrentAsync("replaced"));

        [UnityTest]
        public IEnumerator QueuedPartialLogIsDiscardedWhenDisabled()
            => UniTask.ToCoroutine(() => QueuedPartialLogIsDiscardedWhenItsSourceIsNoLongerCurrentAsync("disabled"));

        [UnityTest]
        public IEnumerator QueuedPartialLogIsDiscardedWhenDestroyed()
            => UniTask.ToCoroutine(() => QueuedPartialLogIsDiscardedWhenItsSourceIsNoLongerCurrentAsync("destroyed"));

        [UnityTest]
        public IEnumerator QueuedPartialLogIsDiscardedWhenLoggingDisabled()
            => UniTask.ToCoroutine(() => QueuedPartialLogIsDiscardedWhenItsSourceIsNoLongerCurrentAsync("logging disabled"));

        private async UniTask QueuedPartialLogIsDiscardedWhenItsSourceIsNoLongerCurrentAsync(string change)
        {
        ...
        private static SileroStreamSpeechDetectorEngine CreateDetector(OnnxSileroStreamSpeechDetector component, SynchronizationContext context)
        {
            ...
            try
            {
                ...
                var create = typeof(OnnxSileroStreamSpeechDetector).GetMethod("CreateDetector", BindingFlags.Instance | BindingFlags.NonPublic);
